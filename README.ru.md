...
RawBridge — open-source CLI и локальный UI-инструмент для подготовки больших RAW-фотоархивов к публикации на сайте.
...
Большие ZIP-скачивания из облаков часто обрываются. RawBridge работает через listing API, скачивает файлы по одному, пишет `.part` файлы, проверяет размер, ведет failed log и умеет retry только failed-файлов.
...
RawBridge использует listing retries, download retries, exponential backoff, `.part` downloads, size checks, failed log, retry only failed и resume без перезаписи готовых outputs.
...
Manifest хранится в `.rawbridge_manifest.sqlite`. Failed files пишутся в `rawbridge_failed.tsv`. Для повторной обработки используйте `--only-failed`.
...
`report.json`, `report.csv`, `errors.csv`, `assets.json`, `report.html`, `picture-snippets.html`, `rawbridge_failed.tsv`.

## Docker

```bash
docker build -t rawbridge:0.1.0 .
docker run --rm -v "$PWD/output:/output" -e DROPBOX_ACCESS_TOKEN="..." rawbridge:0.1.0 rawbridge convert "DROPBOX_LINK" --provider dropbox --out /output
```
...
Запустите `rawbridge doctor`, используйте Python 3.11/3.12, увеличьте retry settings для больших папок, проверьте AVIF support и храните credentials вне репозитория.
