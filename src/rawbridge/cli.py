...
from rawbridge.config import parse_csv_list, parse_int_csv, resolve_preset
from rawbridge.logging import configure_logging
from rawbridge.models import JobConfig
from rawbridge.pipeline.converter import config_from_job_row, run_conversion
from rawbridge.pipeline.manifest import Manifest
from rawbridge.pipeline.scanner import detect_provider

app = typer.Typer(help="RawBridge: local-first RAW archive pipeline for web-ready images.")
console = Console()


def _version_callback(value: bool) -> None:
    if value:
        console.print(f"rawbridge {__version__}")
        raise typer.Exit()
...
def main(
    ...
    """RawBridge command line interface."""
...
def ui(
    ...
    """Start the local RawBridge UI."""
    try:
        from rawbridge.ui.server import run_ui
...
def doctor(out: Path = typer.Option(Path("."), "--out", help="Directory for writeability check.")) -> None:
    ...
    try:
        ...
        probe = out / ".rawbridge_write_test"
...
def _find_manifest_for_job(job_id: str) -> Path | None:
    for path in Path.cwd().rglob(".rawbridge_manifest.sqlite"):
