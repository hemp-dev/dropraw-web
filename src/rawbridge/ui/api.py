...
def create_app():
    ...
    async def ui_token_auth(request: Request, call_next):
        token = os.getenv("RAWBRIDGE_UI_TOKEN")
...
def _validate_scan_request(request: ScanRequest) -> None:
    if _source_looks_local(request.source, request.provider):
        _require_under_roots(Path(request.source), _allowed_roots("RAWBRIDGE_UI_ALLOWED_SOURCE_ROOTS"), "source")


def _validate_job_request(request: JobCreateRequest) -> None:
    if _source_looks_local(request.source, request.provider):
        _require_under_roots(Path(request.source), _allowed_roots("RAWBRIDGE_UI_ALLOWED_SOURCE_ROOTS"), "source")
    _require_under_roots(request.output_dir, _allowed_roots("RAWBRIDGE_UI_ALLOWED_OUTPUT_ROOTS"), "output_dir")
    if request.only_failed is not None:
        _require_under_roots(request.only_failed, _allowed_roots("RAWBRIDGE_UI_ALLOWED_OUTPUT_ROOTS"), "only_failed")
...
def _allowed_roots(env_name: str) -> list[Path]:
    combined = os.getenv(env_name) or os.getenv("RAWBRIDGE_UI_ALLOWED_ROOTS")
    if combined:
