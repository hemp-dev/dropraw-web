...
from rawbridge import __version__
from rawbridge.config import load_presets
from rawbridge.pipeline.scanner import available_provider_metadata, detect_provider
from rawbridge.ui.job_runner import runner
from rawbridge.ui.schemas import JobCreateRequest, JobCreateResponse, ScanRequest, ScanResponse, Settings
...
def create_app():
    ...
    app = FastAPI(title="RawBridge UI API", version=__version__)
    app.add_middleware(
        CORSMiddleware,
    ...
    async def ui_token_auth(request: Request, call_next):
        token = _env_value("RAWBRIDGE_UI_TOKEN", "DROPRAW_UI_TOKEN")
        ...
        if token and request.url.path.startswith("/api/") and not hmac.compare_digest(supplied or "", token):
            return JSONResponse({"detail": "RawBridge UI token required."}, status_code=401)
...
def _request_token(request: object) -> str | None:
    ...
    return (
        headers.get("x-rawbridge-ui-token")
        or cookies.get("rawbridge_ui_token")
    )


def _validate_scan_request(request: ScanRequest) -> None:
    if _source_looks_local(request.source, request.provider):
        _require_under_roots(Path(request.source), _allowed_roots("RAWBRIDGE_UI_ALLOWED_SOURCE_ROOTS", "DROPRAW_UI_ALLOWED_SOURCE_ROOTS"), "source")


def _validate_job_request(request: JobCreateRequest) -> None:
    if _source_looks_local(request.source, request.provider):
        _require_under_roots(Path(request.source), _allowed_roots("RAWBRIDGE_UI_ALLOWED_SOURCE_ROOTS", "DROPRAW_UI_ALLOWED_SOURCE_ROOTS"), "source")
    _require_under_roots(request.output_dir, _allowed_roots("RAWBRIDGE_UI_ALLOWED_OUTPUT_ROOTS", "DROPRAW_UI_ALLOWED_OUTPUT_ROOTS"), "output_dir")
    if request.only_failed is not None:
        _require_under_roots(request.only_failed, _allowed_roots("RAWBRIDGE_UI_ALLOWED_OUTPUT_ROOTS", "DROPRAW_UI_ALLOWED_OUTPUT_ROOTS"), "only_failed")
...
def _env_value(primary: str, legacy: str | None = None) -> str | None:
    return os.getenv(primary) or (os.getenv(legacy) if legacy else None)


def _allowed_roots(primary_env: str, legacy_env: str | None = None) -> list[Path]:
    combined = _env_value(primary_env, legacy_env) or _env_value("RAWBRIDGE_UI_ALLOWED_ROOTS", "DROPRAW_UI_ALLOWED_ROOTS")
    if combined:
...
def _provider_warnings(provider: str, source: str) -> list[str]:
    ...
    if provider == "dropbox" or "dropbox.com" in source:
        warnings.append("RawBridge scans Dropbox folders through the API and does not download the whole folder as ZIP.")
    return warnings
