...
from rawbridge.models import AssetResult, JobSummary
...
def _write_picture_snippets(out_dir: Path, assets: list[AssetResult]) -> None:
    ...
    chunks = [
        "<!doctype html><html><head><meta charset=\"utf-8\"><title>RawBridge Picture Snippets</title></head><body>\n"
    ]
...
def _write_report_html(out_dir: Path, report: dict[str, object], assets: list[AssetResult], summary: JobSummary) -> None:
    ...
    retry_example = (
        f'`rawbridge convert "{summary.source}" --provider {summary.provider} --out "{summary.output_dir}" '
        f"--preset {summary.preset} --only-failed \"{summary.output_dir / 'rawbridge_failed.tsv'}\""
    )
    doc = f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>RawBridge Report {html.escape(summary.job_id)}</title>
  <style>
