...
```text
Open-source CLI + local UI for converting large RAW photo folders from Dropbox, Google Drive, S3/R2 and local folders into optimized WebP/AVIF/JPEG assets — without huge ZIP downloads.
```

Topics:

```text
raw, webp, avif, jpeg, photography, dropbox, google-drive, s3, r2, cli, image-conversion, batch-processing
```
...
```text
Convert RAW photo folders to web-ready images without huge ZIP downloads
```

Subheadline:

```text
Open-source CLI + local UI for Dropbox, Google Drive, S3/R2, and local RAW archives.
```
